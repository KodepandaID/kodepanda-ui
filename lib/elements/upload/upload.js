...
const Upload = ({ id, name, width, accept, disabled, multiple, progress, maxCount, maxSize, content,
  ...
  errorMessage, errorMaxSize, uploadUrl,
  mx, my, mb, ml, mr, mt }) => {
  const node = useRef();
    ...
    f.map((file, i) => {
      if (file.error === undefined) {
        ...
        axios.request({
          ...
          let obj = progressSuccess;
          obj[i] = "completed";
          setProgressSuccess({...obj})
        }).catch(() => {
          let obj = progressSuccess;
          obj[i] = "failed";
          setProgressSuccess({...obj})
    ...
    if (maxSize !== undefined) {
      for (let i = 0; i < f.length; i++) {
        if ((f[i].size / 1000) > maxSize) {
          arr.push({name: f[i].name, size: f[i].size, error: true, errorCause: "maxSize"})
        } else {
          setProgressPercent({...progressPercent, i: 0})
          setProgressSuccess({...progressSuccess, i: "uncompleted"})
          arr.push(f[i])
        }
  ...
  return(
    <div className={wrapperClasses}>
      ...
      onClick={() => {
        ...
        if (progressSuccess === {}) setProgressSuccess({})
      ...
      {files.length > 0 && (
        <div className="flex flex-col space-y-3 mt-4">
          {files.map((file, i) => {
            ...
            return(
              <div key={i} className={listClasses(progressSuccess[i])}>
                <Grid>
                  <Grid.Row>
                    <Grid.Column width="4/5">
                      <span className="text-xs">{file.name} <span className="font-bold">{generateTextSize(file.size)}</span></span>
                    </Grid.Column>
                    <Grid.Column width="1/5"></Grid.Column>
                  </Grid.Row>
                </Grid>
                {progress && (
                  <Progress width="full" percentage={progressPercent[i]} color={progressSuccess[i] === "failed" ? "red" : progressColor} colorContrast={progressColorContrast} bgColor={progressBgColor} bgColorContrast={progressBgColorContrast} />
                )}
                {(file.error !== undefined || progressSuccess[i] === "failed") && (<p className="text-xs text-red-700">{file.errorCause === "maxSize" ? errorMaxSize : errorMessage}</p>)}
              </div>
            )
          })}
        </div>
