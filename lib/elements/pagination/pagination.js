...
const Pagination = ({
  ...
  total, pageSize, defaultCurrent, showSizeChanger, sizeChangerText, sizeChangerWidth, onChange,
  mb, ml, mr, mt,
}) => {
      ...
      <Button.Group>
        {total > 0 && (
          <>
            ...
            {showSizeChanger && (
              <Input.Select width={sizeChangerWidth} placeholder={sizeChanger[0].text} options={sizeChanger} defaultValue={10} size={"sm"} bgColor={color} bgColorContrast={colorContrast} textColor={textColor} textColorContrast={textColorContrast} borderColorContrast={borderColorContrast} onChange={(s) => setTotalPage(Math.floor((total - 1) / s) + 1)} />
            )}
          </>
        )}
        {total === 0 && (
