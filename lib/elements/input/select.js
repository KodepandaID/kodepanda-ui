...
const Select = ({id, name, className, width, value, fluid, disabled, placeholder, size, rounded, onChange, options, shadow,
  ...
  const wrapperClasses = cx(
    (fluid && width === undefined) && "w-full",
    (!fluid && width === undefined) && wrapperSize[size],
        ...
        <ul>
          {options.map((el, index) => {
            return(<li key={el.key} className={itemClasses} onClick={() => listHandleClick(index)}>
              {el.text === inputValue && (
                <Icon className="inline-flex mr-2" icon="check" color={selectHoverColor} colorContrast={600} />
              )}
...
Select.propTypes = {
  ...
  width: PropTypes.oneOf([1, 1.5, 2, 2.5, 3, 3.5, 4, 5, 6, 7, 8, 9, 10, 11, 12, 14, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52]),
