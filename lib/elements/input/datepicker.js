...
const Datepicker = ({ className, id, name, label, width, rounded, roundedPosition, roundedCalendar, fluid, range, time, lang, format,
  ...
  defaultValue, disabled, onChange, shadow, shadowCalendar,
  mt, mb, ml, mr,
  px, py, pt, pb, pl, pr }) => {
  const node = useRef();
  ...
  const [valueUnix, setValueUnix] = useState(0);
  const [valueUnixEnd, setValueUnixEnd] = useState(0);
  ...
  const [dateEndNow, setDateEndNow] = useState(1);
  const [monthEndNow, setMonthEndNow] = useState(0);
  ...
  const selectDate = (d, m, y) => {
    ...
    if (range) {
      if (valueUnix === 0) {
        ...
        if (onChange !== undefined) onChange([valueUnix, valueUnixEnd], [value, valueEnd])
        setOpenCalendar(false)
      } else if (valueUnix > 0 && valueUnixEnd > 0 && valueUnix < t.unix()) {
  ...
  useEffect(() => {
    if (!mounted) {
      ...
      setUnixNow(moment().date(cd.getDate()).month(cd.getMonth()).year(cd.getFullYear()).unix())
